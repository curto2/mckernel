...
namespace lg
{
	...
	template <typename T>
		...
		{
			for (int c = 0; c < _size; c++)
				_data[c] = mean -dev + util::randf() * 2.f * dev;
		}
	...
	////////////////////////////////////////////////////////////
	template <typename T>
		void Tensor<T>::copy(const Tensor<T>& t, const int twidth, const int theight)
		{
			clear();
			_width = twidth;
			_height = theight;
			...
			for (int c = 0; c < _height; c++)
				for (int k = 0; k < _width; k++)
					_data[c * _width + k] = t._data[c * t.width() + k];
		}

	////////////////////////////////////////////////////////////
	template <typename T>
		void Tensor<T>::copy(const Tensor<T>& t, const int expansions)
		{
			clear();
			_width = t._width;
			_height = t._height * expansions;
			_depth = t._depth;
			_depth_size = _width * _height;
			_size = _width * _height;
			_data_secure = std::shared_ptr<T>(new T[_size]);
	...
	template <typename T>
		void Tensor<T>::get_mean_and_variance(float* mean, float* variance)
		{
			...
			if (variance != NULL) {
				for (int c = 0; c < _size; c++)
					tmp_variance += pow(_data[c] - tmp_mean, 2);
	...
	template <typename T>
		...
		{
			for (int c = 0; c < _size; c++)
				if (std::isnan((float)_data[c])) return true;
			return false;
		}
