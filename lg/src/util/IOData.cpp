...
namespace lg
{
	...
	void IOData::writeToStream(std::ofstream& filestream)
	{
		...
		for (int z = 0; z < (int)_subnodes.size(); z++)
			_subnodes[z].writeToStream(filestream);
	}
	...
	{
		for (int z = 0; z < (int)_subnodes.size(); z++)
			if (_subnodes[z].getName() == label) return z;
		return -1;
	}
