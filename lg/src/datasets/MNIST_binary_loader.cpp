...
void MNIST_binary_loader::load_images(std::ifstream& file, std::vector< std::vector<unsigned char> >& images)
{
	...
	for (int c = 0; c < images_count; c++)
		images[c] = std::vector< unsigned char >(images_width * images_height);	
	
	//Read data for each image
	for (int c = 0; c < images_count; c++)
		file.read(reinterpret_cast<char*>(&images[c][0]), sizeof(unsigned char) * images_width * images_height);
}
...
void MNIST_binary_loader::load_labels(std::ifstream& file, std::vector<unsigned char>& labels)
{
	...
	//Read all label values into out buffer
