...
namespace ai
{
		...
		Tensor_float& testingset, Tensor_float& testing_labels)
	{
		...
		std::vector< std::vector< std::string > > CSV_training = ai::CSV_Load(folder_path + "/training_labels.csv");
		std::vector< std::vector< std::string > > CSV_testing = ai::CSV_Load(folder_path + "/testing_labels.csv");
