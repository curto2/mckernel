...
namespace ai 
{
	...
	{
		return std::shared_ptr<Operation>(new Maxpooling(filter_size, stride));	
	}
	
	////////////////////////////////////////////////////////////
	Maxpooling::Maxpooling(const int filter_size, const int stride)
	{
		_input_width = 0;
		_input_height = 0;
		_input_count = 0;
	...
	////////////////////////////////////////////////////////////
	void Maxpooling::initialize(std::vector<Operation*> &inputs)
	{
		...
		//Check inputs have the correct size
	...
	////////////////////////////////////////////////////////////
	void Maxpooling::run(std::vector<Operation*> &inputs, const bool training) 
	{
		//Errors checking
		ensure(inputs.size() == 1);
				...
				for (int x = 0; x < _output_width; x++) {
					...
					for (sx = 0; sx < stopX; sx++) {
						for (sy = 0; sy < stopY; sy++) {
							...
							if (tmp > max) {
								max = tmp;
								_maxin[outid] = f * _input_width * _input_height + _input_width * (inputy + sy) + inputx + sx;
							}
						}
					}
	...
	////////////////////////////////////////////////////////////
	void Maxpooling::backprop(std::vector<Operation*> &inputs) 
	{
		//Errors checking
		ensure(inputs.size() == 1);
		...
		for (int f = 0; f < _input_count; f++) {
			for (int y = 0; y < _output_height; y++) {
				for (int x = 0; x < _output_width; x++) {
					...
					out_errors[_maxin[outid]] += _errors[outid];
	...
	////////////////////////////////////////////////////////////
	void Maxpooling::print()
	{
		printf("Type: Maxpooling, Size: %d, Input: (%dx%dx%d), Output: (%dx%dx%d) Filter_Size: %d, Stride: %d",
