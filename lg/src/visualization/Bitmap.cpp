...
std::string getfileformat(std::string filepath)
{
	...
	for (int z = (int)filepath.size()-1; z >= 0; z--) {
		if (filepath[z] == '.') {
			dot_pos = z;
			break;
		}
	...
	//Get format
	for (int z = dot_pos+1; z < (int)filepath.size(); z++)
		fmt += filepath[z];
...
Bitmap::Bitmap(std::string filepath, Channels channels)
{
	...
	else if (fmt == "pgm")
	{
		...
		//Set channel information, PGM format support only mono channel
		m_channels = 1;
...
void Bitmap::fill(long color)
{
	...
	for (int z = 0; z < m_width * m_height; z++)
		for (int r = 0; r < m_channels; r++)
			m_data[z * m_channels + r] = c[r];	
	...
	for (int x = 0; x < m_width; x++) {
		for (int y = 0; y < m_height; y++) {
			...
			for (int z = 0; z < m_channels; z++)
				color += m_data[(y * m_width + x) * m_channels + z];
			color /= m_channels;
...
void Bitmap::adjustContrast(float contrast)
{
	for (int x = 0; x < m_width; x++) {
		for (int y = 0; y < m_height; y++) {
			...
			static int pos = 0;
			for (int z = 0; z < m_channels; z++) {
				pos = (y * m_width + x) * m_channels + z;
				c[z] = m_data[pos] / 255.0f;
				c[z] = (((c[z] - 0.5f) * contrast) + 0.5f) * 255.0f;
				if (c[z] > 255) c[z] = 255;
				else if (c[z] < 0) c[z] = 0;
				m_data[pos] = (int)c[z];
			}
...
void Bitmap::resize(int w, int h)
{
	...
	//Copy resized data into new array
	for (int x = 0; x < w; x++)
		for (int y = 0; y < h; y++)
...
void Bitmap::rotate(const unsigned int center_x, const unsigned int center_y, const float angle, const long fill_color)
{
	...
	for (int z = 0; z < m_width * m_height * m_channels; z++) m_data[z] = temp[z];
...
{
	for (int z = 0; z < m_width * m_height * m_channels; z++) {
		if (m_data[z] < threshold) m_data[z] = 0;
		else m_data[z] = 255;
	}
}
		...
		const unsigned int height, const long color, const unsigned int border_thickness)
{
	...
	if (border_thickness <= 0) //fill rectangle with color
	{
		for (unsigned int z = x; z < x + width; z++) {
			if (z >= m_width) continue;
			for (unsigned int r = y; r < y + height; r++) {
				if (r >= m_height) continue;
				for (unsigned int c = 0; c < m_channels; c++) {
					m_data[(r * m_width + z) * m_channels + c] = GETBYTE(color, (m_channels -1 -c)); 
				}
			}
		}
	}
	else //Draw the borders of the rectangle
	{
		...
		//Draw y axis borders
		for (int r = (int)y - offset_l; r < (int)y + (int)height + offset_h; r++) {
			if (r < 0 || r >= m_height) continue;
			for (int z = (int)x -offset_l; z < (int)x + offset_h; z++) {
				if (z < 0 || z >= m_width) continue;
				for (int c = 0; c < m_channels; c++) {
					m_data[(r * m_width + z) * m_channels + c] = GETBYTE(color, (m_channels -1 -c)); 
				}
			}
			for (int z = (int)x + (int)width -offset_l; z < (int)x + (int)width + offset_h; z++) {
				if (z < 0 || z >= m_width) continue;
				for (int c = 0; c < m_channels; c++) {
					m_data[(r * m_width + z) * m_channels + c] = GETBYTE(color, (m_channels -1 -c)); 
				}
		...
		//Draw x axis borders
		for (int z = (int)x - offset_l; z < (int)x + (int)width + offset_h; z++) {
			if (z < 0 || z >= m_width) continue;
			for (int r = (int)y - offset_l; r < (int)y + offset_h; r++) {
				if (r < 0 || r >= m_height) continue;
				for (int c = 0; c < m_channels; c++) {
					m_data[(r * m_width + z) * m_channels + c] = GETBYTE(color, (m_channels -1 -c)); 
				}
			}
			for (int r = (int)y + (int)height -offset_l; r < (int)y + (int)height + offset_h; r++) {
				if (r < 0 || r >= m_height) continue;
				for (int c = 0; c < m_channels; c++) {
					m_data[(r * m_width + z) * m_channels + c] = GETBYTE(color, (m_channels -1 -c)); 
				}
		...
		const long color, const unsigned int border_tickness)
{
	...
	{
		for (int z = (int)x - (int)radius; z < (int)x + (int)radius; z++) {
			if (z < 0 || z >= m_width) continue;
			for (int r = (int)y - (int)radius; r < (int)y + (int)radius; r++) {
				if (r < 0 || r >= m_height) continue;
				if (sqrt(pow(z - (int)x, 2) + pow(r - (int)y, 2)) > radius) continue;
				for (int c = 0; c < m_channels; c++)
					m_data[(r * m_width + z) * m_channels + c] = GETBYTE(color, (m_channels -1 -c));
			}
		}
	...
	{
		for (int z = (int)x - (int)radius; z < (int)x + radius; z++) {
			if (z < 0 || z >= m_width) continue;
			for (int r = (int)y - (int)radius; r < (int)y + radius; r++) {
				if (r < 0 || r >= m_height) continue;
				const unsigned int distance = sqrt(pow(z - (int)x, 2) + pow(r - (int)y, 2));
